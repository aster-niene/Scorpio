...
// ==== ESP32 + PCA9685 adaptation ====
// Two PCA9685 drivers for 18 servos
Adafruit_PWMServoDriver pwm1 = Adafruit_PWMServoDriver(0x40);
Adafruit_PWMServoDriver pwm2 = Adafruit_PWMServoDriver(0x41);

// Convert microseconds to PCA9685 ticks at 50 Hz
static inline uint16_t usToTicks(uint16_t us) {
  // 50Hz => period = 20000us; PCA9685 has 4096 ticks per period
  // ticks = us * 4096 / 20000
  return (uint16_t)((us * 4096UL) / 20000UL);
}

// Minimal wrapper so existing code (attach/write) still works
class MyServo {
public:
  MyServo(): _driverIdx(0), _ch(0), _minUS(1000), _maxUS(2000) {}
  void attachChannel(uint8_t driverIdx, uint8_t ch, uint16_t minUS=610, uint16_t maxUS=2400) {
    _driverIdx = driverIdx; _ch = ch; _minUS = minUS; _maxUS = maxUS;
    write(90); // center
  }
  // Kept for compatibility with original attach signature, ignored
  void attach(int /*pin*/, uint16_t minUS=610, uint16_t maxUS=2400) {
    _minUS = minUS; _maxUS = maxUS;
  }
  void write(int angle) {
    if(angle < 0) angle = 0;
    if(angle > 180) angle = 180;
    uint16_t us = (uint16_t)(_minUS + (long)(angle) * (long)(_maxUS - _minUS) / 180L);
    Adafruit_PWMServoDriver* drv = (_driverIdx==0) ? &pwm1 : &pwm2;
    drv->setPWM(_ch, 0, usToTicks(us));
  }
private:
  uint8_t _driverIdx, _ch;
...
//***********************************************************************
const int BATT_VOLTAGE = 34;           //12V Battery analog voltage input port

const int PS2_DAT = 19;                //gamepad port definitions
const int PS2_ATT = 5;
const int PS2_CMD = 23;
const int PS2_CLK = 18;
...
const float HOME_Z[6] = { -60.0, -60.0, -60.0,  -60.0,  -60.0, -60.0};

const float BODY_X[6] = { 66.0,  0.0, -66.0, -66.0,    0.0, 66.0};  //body center-to-coxa servo distances 
...
void setup()
{
  ...
  // ADC attenuation for battery input
  analogSetPinAttenuation(BATT_VOLTAGE, ADC_11db);
  
  //attach servos  coxa1_servo.attachChannel(0,0,610,2400);  femur1_servo.attachChannel(0,1,610,2400);  tibia1_servo.attachChannel(0,2,610,2400);  coxa2_servo.attachChannel(0,3,610,2400);  femur2_servo.attachChannel(0,4,610,2400);  tibia2_servo.attachChannel(0,5,610,2400);  coxa3_servo.attachChannel(0,6,610,2400);  femur3_servo.attachChannel(0,7,610,2400);  tibia3_servo.attachChannel(0,8,610,2400);  coxa4_servo.attachChannel(0,9,610,2400);  femur4_servo.attachChannel(0,10,610,2400);  tibia4_servo.attachChannel(0,11,610,2400);  coxa5_servo.attachChannel(0,12,610,2400);  femur5_servo.attachChannel(0,13,610,2400);  tibia5_servo.attachChannel(0,14,610,2400);  coxa6_servo.attachChannel(0,15,610,2400);  femur6_servo.attachChannel(1,0,610,2400);  tibia6_servo.attachChannel(1,1,610,2400);//connect the gamepad
...
void compute_strides()
{
  ...
  if(gait_speed == 0) duration = 1080; // (960, 840, 720, 600, 480) 
  else duration = 3240;
}
...
void LED_Bar(int LED_color,int LED_count) {
  // LED bar disabled for ESP32 build
}
